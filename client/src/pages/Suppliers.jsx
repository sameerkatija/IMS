...
import { Truck, Plus, Edit2, Search, FileText, Phone, MapPin, ShieldCheck } from "lucide-react";
...
const Suppliers = () => {
  ...
  const handleReconcile = async (supplier) => {
    try {
      setReconcileLoading(true);
      const response = await api.get(`/api/supplier/${supplier.id}/reconcile`);
      if (response.data && response.data.type === "success") {
      ...
      console.error(err);
      setToast({ message: "Failed to reconcile supplier ledger.", type: "error" });
    } finally {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="w-full md:w-56 flex items-center bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2">
          ...
          >
            ...
            <option value="weOwe">We Owe Them (Owed by Us)</option>
            <option value="oweUs">Owes Us (Supplier owes us)</option>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-xl space-y-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Account details and transaction verification for <strong>{reconcileData.supplierName}</strong>.
                </p>
              </div>
